( () => {
    var e = {
        797: (e, t, n) => {
            ...
            var s = n(933);
            var i = n.n(s);
            //Clases importadas y ajustadas CLIENTE
            ...
            let isFollowingBot = false; // Flag global para controlar el seguimiento al bot
            ...
            // Reconstruir células (instancias de Mt)
            function reconstructMt(data) {
                const parentPlayer = {
                    id: data.parentPlayer.id,
                    parentClientID: data.parentPlayer.parentClientID,
                    parentClient: {
                        nick: data.parentPlayer.parentClient.nick,
                        tag: data.parentPlayer.parentClient.tag,
                        isBot: data.parentPlayer.parentClient.isBot
                    },
                    skinURL: data.parentPlayer.skinURL
                };
            ...
            class M {
                ...
                update() {
                    ...
                    if (R.isAlive) {
                        this.x += (R.x - this.x) / (30 - h.cameraSpeed);
                        this.y += (R.y - this.y) / (30 - h.cameraSpeed)
                    } else {
            ...
            class ce {
                ...
                /*updateSortedCells(reconstructedCells) {
                    if (!Array.isArray(reconstructedCells)) {
                        console.error("Invalid input: reconstructedCells must be an array.");
                        return;
                    }
            ...
            class Te {
                ...
                render(e) {
                    ...
                    Wt.sortedCells = reconstructedCells;
            ...
            class Me {
                ...
                sort() {
                    ...
                    for (const [t, n] of Wt.cells) {
                        // Ignorar células que no sean del jugador o que sean del propio jugador
                        ...
                        // Clasificar las células según su masa
                        if (t > e * 2 * 1.3)
                            s = 0;
                        else if (t > e * 1.3)
                            s = 1;
                            ...
                            s = 2;
                        else if (t > e * 0.5 * 0.77)
                            s = 3;
                        else if (t > e / 16 * 1.3)
                            s = 4;
                
                        // Añadir la célula al nivel correspondiente
            ...
            class Ae {
                ...
                /*
                setCamera() {
                    const transitionSpeed = 2; // Ajusta la velocidad de transición
                    if (((!R.isAlive && !statusbotcell) && !isZActive) || isTabActive && !statusbotcell) {
                        // Activar el modo de seguimiento al bot
                        isFollowingBot = true;

                        // Interpolar hacia las coordenadas del bot
                        E.x += (Ebot.x - E.x) * transitionSpeed;
                        E.y += (Ebot.y - E.y) * transitionSpeed;

                        // Interpolar zoom si es necesario
                        //E.zoom += (Ebot.zoom - E.zoom) * transitionSpeed;
                    } else {
                        // Si no se cumplen las condiciones, desactivar el modo de seguimiento al bot
                        isFollowingBot = false;
                    }

                    // Lógica normal para la cámara
                    const e = E.zoom * h.graphicsQuality;
                    const t = (this.canvas.width >> 1) / e - E.x;
                    const n = (this.canvas.height >> 1) / e - E.y;
                    this.ctx.scale(e, e);
            ...
            class wt {
                ...
                /*setCamera() {
                    const transitionSpeed = 2; // Velocidad de interpolación
                
                    if (((!R.isAlive && !statusbotcell) && !isZActive) || isTabActive && !statusbotcell) {
                        // Interpolar las coordenadas hacia las del bot
                        E.x += (Ebot.x - E.x) * transitionSpeed;
                        E.y += (Ebot.y - E.y) * transitionSpeed;
                
                        // Opcional: interpolar zoom si es necesario
                        E.zoom += (Ebot.zoom - E.zoom) * transitionSpeed;
                    }
                
                    // Calcular escala y transformaciones
                    const e = E.zoom * h.graphicsQuality;
                    const t = this.canvas.width / 2 - E.x * e;
                    const n = this.canvas.height / 2 - E.y * e;
            ...
            class Tt {
                constructor() {
                    ...
                    // Detecta cuando el jugador muere y activa el modo espectador automáticamente
                    this.register(y.Player_Died, (() => {
                        if (!R.isAlive) { // Si la célula está muerta
                            V.onSpectate(); // Configuración visual del modo espectador
                            R.isSpectating = true; // Activa el estado de espectador
                        }
                    }));
                    // Mantiene la lógica para cuando se solicita jugar manualmente
                    this.register(y.Request_Play, (() => {
                        V.onPlay();
                        if (R.isAlive) return;
                        An.spawn(); // Solo reaparece si no está vivo
                    }));
                    // Mantiene la lógica para cuando se solicita entrar en modo espectador manualmente
                    this.register(y.Request_Spectate, (() => {
                        V.onSpectate();
                        if (!R.isAlive) R.isSpectating = true;
                    }));
            ...
            class In {
                ...
                connect(e, t="") {
                    ...
                    this.isReplay = false;
                    self.game_server = e;
                    webSocketInstance.sendserver(e);
                    this.url = e;
