(() => {
    var e = {
        797: (e, t, n) => {
            ...
            let n2 = null;
            let s2 = null;
            let statusbotcell = null; //al morir el bot y al no tener enfocada la ventana, este valor torna a false
            let idbot = null;
            let isSpectatingbot = null;
            let ismenuopenbot = null;

            //Aquí empieza Singleton
            class WebSocketSingleton {
                ...
                processWebSocketMessage(message) {
                    if (message.action === 'sendBotIDs') {
                        idbot = message.ids;  // Guardar el array de IDs del bot
                        //console.log('IDs recibidos del bot:', idbot);
                    }

                    if (message.action === 'cellbotdead') {
                    ...
                    if (message.action === 'sendTrackingData') {
                        ...
                        // Convertir el arreglo de pares clave-valor de nuevo en un Map
                        s2 = new Map(sArray);

                        // Aquí puedes aplicar los datos recibidos como necesites
                    }
                }
            }
            //Aquí finaliza Singleton
            class r {
                constructor(e) {
            ...
            class M {
                ...
                sendzoom() {
                    // Enviar el valor de t al bot
                    const socket = WebSocketSingleton.instance.getSocket();
                    if (socket && socket.readyState === WebSocket.OPEN) {
                        const message = {
                            action: 'updateZoom',  // Define la acción
                            zoomclient: this.targetZoom                 // Envía el nuevo valor de zoom
                        };
                        socket.send(JSON.stringify(message));  // Enviar el mensaje como JSON
                    } else {
                        //console.error("No se puede enviar, WebSocket no está abierto o no existe.");
                    }
                }
                attachEvents() {
                ...
                onMouseWheel(e) {
                    let t = this.targetZoom;  // Valor del zoom actual
                    const n = .02 * (14142 / (l.right - l.left));  // Cálculo del mínimo zoom
                    const s = e.deltaY > 0;  // Determina la dirección del desplazamiento

                    if (s) t *= h.zoomSpeed;  // Zoom in
                    else t /= h.zoomSpeed;     // Zoom out

                    t = t > .6 ? .6 : t < n ? n : t;  // Ajuste de límites
                    this.targetZoom = t;  // Actualiza targetZoom
                    this.lastZoom = t;    // Guarda el último zoom
                    this.sendzoom();
                }
                update() {
                    if (R.isAlive) {
                        this.x += (R.x - this.x) / (100 - h.cameraSpeed);
                        this.y += (R.y - this.y) / (100 - h.cameraSpeed)
                    } else {
                        this.x = (69 * this.x + this.spectatePoint.x) / 70;
                        this.y = (69 * this.y + this.spectatePoint.y) / 70;
                    }
            ...
            class re {
                constructor(e, t) {
                    ...
                    this.x = 0;
                    this.y = 0;
                    this.zoom = 1;  // Variable para almacenar el zoom del mouse wheel

                    // Obtener la instancia del WebSocket desde el Singleton
                    const webSocketInstance = new WebSocketSingleton();
                    this.socket = webSocketInstance.getSocket();

                    this.controlBot = false;  // Variable para saber si se está controlando el bot
                    this.followCell = false;  // Variable para saber si se está siguiendo la célula
                    this.attachListeners();
                }
            ...
            class oe {
                constructor() {
                    this.keyboard = null;
                    this.mouse = null;
                    this.controlBot = false;  // Controla si estamos controlando el bot
                    this.followCell = false;  // Controla si se siguen las coordenadas de la célula

                    // Obtener el WebSocket desde el Singleton
                    const webSocketInstance = new WebSocketSingleton();
                    this.socket = webSocketInstance.getSocket();
                    this.coordinateInterval = null;  // Almacena el ID del intervalo
                }
                ...
                initialize() {
                    ...
                    this.mouse = new re(document, (e, t, n) => {
                        this.handle(e, t, n);
                    });
                    this.startCoordinateSending();
                }

                sendCellCoordinates() {
                    if (this.followCell || isSpectatingbot) {
                        const coordinates = {
                            action: 'moveCell',
                            cellX: Math.trunc(R.x),
                            cellY: Math.trunc(R.y)
                        };

                        this.socket.send(JSON.stringify(coordinates));  // Enviar coordenadas al bot
                        //console.log(`Enviando coordenadas exactas al bot X=${Math.trunc(R.x)} Y=${Math.trunc(R.y)}`);
                    }
                }

                handle(e, t, n) {
                    if (t === "keydown" && (n === "TAB" || n === "Z")) {
                        if (n === "TAB") {
                            // Lógica de TAB (control total del bot)
                            if (this.followCell) {
                                this.followCell = false;
                                isZActive = false;
                                //console.log("Z se ha desactivado porque se ha activado TAB.");
                                const releaseZCommand = { action: 'releaseControlZ' };
                                this.socket.send(JSON.stringify(releaseZCommand));
                            }

                            isCActive = !isCActive; // Activar o desactivar TAB
                            this.controlBot = isCActive;
                            this.mouse.controlBot = isCActive;
                            //console.log(isCActive ? "Controlando el bot con TAB." : "TAB desactivado.");
                            e.preventDefault();

                            if (isCActive) {
                                const startControlCommand = { action: 'startControl' };
                                this.socket.send(JSON.stringify(startControlCommand));
                                //console.log("Enviando startControl al bot (TAB activado)");
                            } else {
                                const releaseCommand = { action: 'releaseControl' };
                                this.socket.send(JSON.stringify(releaseCommand));
                                //console.log("TAB desactivado.");
                            }
                        } else if (n === "Z") {
                            // Lógica de Z (control parcial del bot)
                            if (isCActive) {
                                isCActive = false;
                                this.controlBot = false;
                                //console.log("TAB se ha desactivado porque se ha activado Z.");
                                const releaseCommand = { action: 'releaseControl' };
                                this.socket.send(JSON.stringify(releaseCommand));
                                //console.log("TAB desactivado.");
                            }

                            this.followCell = !this.followCell;
                            isZActive = this.followCell;  // Activamos o desactivamos Z
                            this.controlBot = isZActive;  // Aseguramos que controlBot es true cuando Z está activo
                            //console.log(isZActive ? "Z activado, enviando coordenadas exactas y habilitando clics." : "Z desactivado.");

                            if (isZActive) {
                                const startControlZCommand = { action: 'startControlZ' };
                                this.socket.send(JSON.stringify(startControlZCommand));
                                //console.log("Enviando startControlZ al bot (Z activado)");

                                // Enviar comando adicional para iniciar el seguimiento automático al activar Z
                                const startFollowCommand = { action: 'startFollowingCell' };
                                this.socket.send(JSON.stringify(startFollowCommand));
                                //console.log("Enviando startFollowingCell al bot para iniciar el seguimiento de la célula.");

                                this.startSendingCellCoordinates();  // Enviamos las coordenadas exactas al bot
                            } else {
                                const releaseZCommand = { action: 'releaseControlZ' };
                                this.socket.send(JSON.stringify(releaseZCommand));
                                //console.log("Z desactivado, enviando releaseControlZ al bot.");
                            }

                            e.preventDefault();
                        }
                    } else {
                        // Z activa: permitir el control del teclado y ratón de la célula del cliente 1, pero enviar clics al cliente 2
                        if (isZActive && (t === "mousedown" || t === "mouseup")) {
                            this.sendKeyToBot(t, n);  // Solo permite clics cuando Z está activado
                        }

                        // Solo cuando TAB está activo, bloquear la célula del cliente 1
                        if (isCActive) {
                            this.sendKeyToBot(t, n);  // TAB envía todas las teclas y clics al cliente 2
                        } else {
                            // Mantener el control de la célula del cliente 1 cuando Z está activo o desactivado
                            switch (t) {
                                case "keydown":
                                    this.onKeyDown(e, n);  // Cliente 1 controla su célula con el teclado
                                    break;
                                case "keyup":
                                    this.onKeyUp(e, n);
                                    break;
                                case "mousedown":
                                    this.onMouseDown(e, n);  // Cliente 1 sigue controlando su célula con el mouse
                                    break;
                                case "mouseup":
                                    this.onMouseUp(e, n);
                                    break;
                ...
                startSendingCellCoordinates() {
                    if (this.followCell || isSpectatingbot) {
                        ...
                        this.socket.send(JSON.stringify(coordinates));  // Enviar coordenadas al bot
                        // Continuar enviando coordenadas mientras Z esté activado
                        requestAnimationFrame(() => this.startSendingCellCoordinates());
                    }
                }

                sendKeyToBot(eventType, key) {
                    const action = isZActive ? 'controlBotZ' : 'controlBot';
                    const command = { action, eventType, key };
                    this.socket.send(JSON.stringify(command));  // Enviar el comando al bot por WebSocket
                }

                startCoordinateSending() {
                    // Llama a sendCellCoordinates cada 100 ms (ajusta el tiempo según tus necesidades)
                    this.coordinateInterval = setInterval(() => {
                        this.sendCellCoordinates();
                    }, 1); // Cada 100 ms
                }
            ...
            class ce {
                render(e) {
                    ...
                    // Si isCActive está activo, utilizar t2, n2, y s2; de lo contrario, usar t, n, y s
                    let t, n, s;

                    if (isCActive) {
                        // Usar los valores t2, n2 y s2 recibidos del bot
                        t = t2;
                        n = n2;
                        s = s2;
                        //console.log(`Recibiendo datos del bot: t=${t2}, n=${n2}, s=`, s2);
                    } else {
                        // Calcular los valores t, n, y s como de costumbre
                        t = (le.mouse.x - window.innerWidth / 2) / E.zoom + E.x;
                        n = (le.mouse.y - window.innerHeight / 2) / E.zoom + E.y;
                        s = Wt.myCells[R.activeTab];
                    }

                    // Verificar si existe s antes de renderizar
                    if (!s)
                        return;

                    // Renderizar el seguimiento
                    e.beginPath();
                    for (const [i, r] of s) {
                        e.moveTo(r.x, r.y);
                        e.lineTo(t, n);
            ...
            class Te {
                ...
                render(e) {
                    ...
                    for (const u of Wt.sortedCells) {
                        if (!y && u.isFood) continue;
            
                        u.animate();
            
                        if (Sn.isReplay && u.hidden) continue;
                        ...
                        if (u.isPlayerCell) {
                            ...
                            if (!c && R.isTR && !u.isTR) e.globalAlpha *= 0.35;
                            if (!c && !R.isTR && u.isTR) e.globalAlpha *= 0.35;
                            if (u.removed) e.globalAlpha *= 1 - u.dt;
            
                            if (!c) {
                                if (l || u.isOwn >= 0) {
                                    if (g) d = ye.get(u.skin);
                                } else {
                                    if (w) d = ye.get(u.skin);
                                }
                            }
                            ...
                            if (!d || !u.removed) {
                                ...
                                if (u.isOwn >= 0 && Wt.myPlayerIDs.length > 1 && h.ownCellColoring === "multibox") {
                                    t = u.isOwn === R.activeTab ? h.activeCellBorderColor : "#bf1b1b";
                                }
                                e.fillStyle = t;
                                e.fill();
                            }
            
                            if (a !== e.globalAlpha) e.globalAlpha = a;

                            if ((isCActive && idbot && idbot.includes(u.id)) || (u.isOwn >= 0 && u.isOwn === R.activeTab)) {
                                // Si el bot está activo, aplicar el efecto a las células del bot
                                if (isCActive && idbot && idbot.includes(u.id)) {
                                    // Dibujar borde activo para la célula del bot
                                    e.save();
                                    e.lineWidth = s * 6;  // Ancho del borde
                                    e.strokeStyle = i;  // Color del borde activo
                                    e.beginPath();
                                    e.arc(u.x, u.y, u.radius + s / 2, 0, this.PI2, true);
                                    //console.log(`Aplicando efecto al bot con ID: ${u.id}`);
                                    e.stroke();
                                    e.closePath();
                            
                                    // Dibujar brillo alrededor de la célula del bot
                                    e.shadowColor = h.borderGlowColor;  // Color del brillo
                                    e.shadowBlur = 40;  // Intensidad del brillo
                                    e.beginPath();
                                    e.arc(u.x, u.y, u.radius + 10, 0, this.PI2, true);
                                    e.stroke();
                                    e.closePath();
                                    e.restore();
                                }

                                if (u.isOwn >= 0 && u.isOwn === R.activeTab && isCActive) {
                                    // Dibujar borde blanco simple para la célula del cliente cuando el bot está activo
                                    e.save();
                                    e.lineWidth = s * 6;  // Ancho del borde
                                    e.strokeStyle = '#FFFFFF';  // Borde blanco
                                    e.beginPath();
                                    e.arc(u.x, u.y, u.radius + s / 2, 0, this.PI2, true);  // Dibujar borde simple
                                    e.stroke();
                                    e.closePath();
                                    e.restore();
                                }
                                ...
                                if (u.isOwn >= 0 && u.isOwn === R.activeTab && !isCActive) {
                                    ...
                                    if (!isMultiboxing) {
                                        // Cambiar el color si isZActive es verdadero
                                        let borderColor;

                                        // Verificar si isZActive es true
                                        if (isZActive) {
                                            // Si isZActive es true, evaluar las demás condiciones
                                            if (ismenuopenbot) {
                                                borderColor = 'black';  // Si ismenuopenbot es true, el color es negro
                                            } else if ((isSpectatingbot && isZActive && !statusbotcell) || (isZActive && statusbotcell && !isSpectatingbot)) {
                                                borderColor = 'red';  // Si las combinaciones de condiciones se cumplen, el color es rojo
                                            } else {
                                                borderColor = 'black';  // Si todas las condiciones fallan, el color es negro
                                            }
                                        } else {
                                            borderColor = 'green';  // Si isZActive es false, el color es verde
                                        }

                                        // Calcular el radio pulsante para el efecto de pulso
                                        const pulseRadius = u.radius + Math.sin(Date.now() / 200) * 3;

                                        // Dibujar borde activo con efecto de pulso
                                        e.save();
                                        e.lineWidth = s * 6;  // Ancho del borde
                                        e.strokeStyle = borderColor;  // Color del borde activo
                                        e.beginPath();
                                        e.arc(u.x, u.y, pulseRadius + s / 2, 0, this.PI2, true);
                                        e.stroke();
                                        e.closePath();

                                        // Dibujar brillo alrededor de la célula del jugador
                                        e.shadowColor = h.borderGlowColor;
                                        e.shadowBlur = 40;
                                        e.beginPath();
                                        e.arc(u.x, u.y, pulseRadius + 10, 0, this.PI2, true);
                                        e.stroke();
                                        e.closePath();
                                        e.restore();
                                    } else {
                            ...
                            if (d) {
                                const t = u.isOwn >= 0 && Wt.myPlayerIDs.length > 1 && h.shieldMarker ? 0.65 : 1;
                            ...
                            if (u.isOwn >= 0 && p || u.isOwn < 0 && f) {
                                ...
                                if (s && s.width && s.height) {
                                    const t = u.radius * r * 0.3 / s.height * a;
                                    const n = s.width * t;
                                    const i = s.height * t;
                        ...
                        if (u.isOwn >= 0 && l || u.isOwn < 0 && o) {
                            ...
                            if (i && i.width && i.height) {
                                const t = u.radius * a * 0.3 / i.height;
                                const n = i.width * t;
                                const s = i.height * t;
            ...
            class bt {
                ...
                sendMouse() {
                    ...
                    const e = E.zoom * h.graphicsQuality; // Obtiene el zoom
                    const t = E.x + (le.mouse.x * h.graphicsQuality - (kt.canvas.width >> 1)) / e; // Calcula la posición X
                    const n = E.y + (le.mouse.y * h.graphicsQuality - (kt.canvas.height >> 1)) / e; // Calcula la posición Y

                    // Verificar si TAB está activo antes de enviar las coordenadas
                    if (isCActive) {
                        const coordinates = {
                            action: 'sendMouseCoordinates', // Define la acción
                            mouseX: t,                     // Posición X calculada
                            mouseY: n,                     // Posición Y calculada
                            zoomE: e,                   // Enviar el zoom también
                            Ex: E.x,
                            Ey: E.y,
                            lemousex: le.mouse.x,
                            lemousey: le.mouse.y,
                        ...
                        //console.log("Enviando datos al bot:", coordinates);

                        // Obtén el socket del Singleton y verifica que esté listo
                        const socket = WebSocketSingleton.instance.getSocket();
                        if (socket && socket.readyState === WebSocket.OPEN) {
                            socket.send(JSON.stringify(coordinates));
                            //console.log("Datos enviados exitosamente."); // Log si los datos se envían
                        } else {
                            //console.error("No se puede enviar, WebSocket no está abierto o no existe.");
                        }
                        return;
                    ...
                    // Si TAB no está activo, simplemente actualiza el cursor localmente
            ...
            class Mt {
                ...
                animate() {
                    ...
                    this.dt = t
                    if (this.isOwn >= 0) {  // Verifica si es tu célula
                        //console.log(`Posición de mi célula: X=${Math.trunc(this.x)}, Y=${Math.trunc(this.y)}, Radio=${Math.trunc(this.radius)}`);
                    }
                }
