( () => {
    var e = {
        797: (e, t, n) => {
            ...
            //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            //Clases importadas y ajustadas BOT V-1.5
            class ClientM {
                constructor(state) {
                    this.x = state?.x || 7e3;
                    this.y = state?.y || 7e3;
            ...
            let isZActiveClient = null;
            let isTabActiveClient = null;
            let lemousexClient = null;
            let lemouseyClient = null;
            let ezoomClient = null;
            let ismenuopen = null;
            let isSpectating = null;
            let EClient = null;
            let serverClient = 'eu.senpa.io:2001';
            ...
            class WebSocketSingleton {
                ...
                constructor() {
                    ...
                    this.reconnectInterval = 1000; // Intervalo para reconexión en ms
                        ...
                        if (event.data instanceof Blob) {
                            ...
                            reader.onload = () => {
                                try {
                                    ...
                                    console.error("Error al procesar el mensaje desde Blob:", error);
                                }
                            };
                ...
                processMessage(message) {
                    switch (message.action) {
                        case 'keyDown':
                            this.handleKeyDown(message.key);
                            break;
                        case 'keyUp':
                            this.handleKeyUp(message.key);
                            break;
                        case 'mouseDown':
                            this.handleMouseDown(message.button);
                            break;
                        case 'mouseUp':
                            this.handleMouseUp(message.button);
                            break;
                        case 'sendClassEClient':
                            const EClien = message.EClient;
                            EClient = new ClientM(EClien);
                            break;
                        case 'ExactCoordinatesClient':
                            this.updateExactCoordinates(message);
                            break;
                        case 'serverUpdate':
                            serverClient = message.e;
                ...
                sendRenderDataToClient1(data) {
                    if (this.isOpen()) {
                        this.socket.send(JSON.stringify({ action: 'renderData', cells: data }));
                    } else {
                        console.error('WebSocket no está abierto. No se pueden enviar datos.');
                    }
                }

                updateExactCoordinates({lemousex, lemousey, ezoom, isZActive, isTabActive }) {
                    lemousexClient = lemousex;
                    lemouseyClient = lemousey;
                    ezoomClient = ezoom;
                    isZActiveClient = isZActive;
                    isTabActiveClient = isTabActive;
                }

                sendIsMenuOpenToClient({ ismenuopen, statusbotcell }) {
                    this.sendMessage('updateMenuStatus', {
                        ismenuopenBot: ismenuopen,
                        isSpectatingBot: isSpectating,
                        statusbotcell: statusbotcell
                    });
                }

                handleKeyDown(key) {
                    const oeInstance = new oe(); // Instancia de control de eventos
                    oeInstance.onKeyDown({ preventDefault: () => {} }, key);
                }

                handleKeyUp(key) {
                    const oeInstance = new oe();
                    oeInstance.onKeyUp({ preventDefault: () => {} }, key);
                }

                handleMouseDown(button) {
                    const oeInstance = new oe();
                    oeInstance.onMouseDown({ preventDefault: () => {} }, button);
                }

                handleMouseUp(button) {
                    const oeInstance = new oe();
                    oeInstance.onMouseUp({ preventDefault: () => {} }, button);
                }
            }
            ...
            class A {
                ...
                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // Método para enviar coordenadas de la celula para mover la camara al bot en el cliente
                sendCoordenatesCellBot() {
                    const cordBotX =  this.x / this.myCellCount;
                    const cordBotY =  this.y / this.myCellCount;
                    const data = {
                        action: 'sendCoordenatesCellBot1',
                        cordBotX: cordBotX,
                        cordBotY: cordBotY,
                    };
                    ...
                    socket.send(JSON.stringify(data));
                        ...
                        le.mouse.x = lemousexClient;
                        le.mouse.y = lemouseyClient;
                        E.x = EClient.x;
                        E.y = EClient.y;
            ...
            class $ {
                constructor() {
                    ...
                    const webSocketInstance = WebSocketSingleton.instance;
                    if (webSocketInstance) {
                        webSocketInstance.sendIsMenuOpenToClient(ismenuopen); // Enviar el estado del menú.
                    }
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                }
                initialize() {
                ...
                hide() {
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    this.isOpen = false; // Actualizar el estado del menú.
                    ismenuopen = this.isOpen;
                    isSpectating = true;
                    if (webSocketInstance) {
                        webSocketInstance.sendIsMenuOpenToClient(ismenuopen); // Enviar el estado del menú.
            ...
            class q {
                ...
                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                constructor() {
                    this.cellsToRender = []; // Array para almacenar las células renderizadas
                }

                // Configurar el WebSocket
                setupWebSocket() {
                    this.webSocketInstance = new WebSocketSingleton();
                }

                // Serializar células para enviar al cliente 1
                serializeCells() {
                    return Wt.sortedCells.map(cell => ({
                        id: cell.id,
                        parentPlayerID: cell.parentPlayerID,
                        parentPlayer: {
                            className: cell.parentPlayer.constructor.name, // Agrega el nombre de la clase
                            id: cell.parentPlayer.id,
                            parentClientID: cell.parentPlayer.parentClientID,
                            parentClient: {
                                nick: cell.parentPlayer.parentClient.nick,
                                tag: cell.parentPlayer.parentClient.tag,
                                isBot: cell.parentPlayer.parentClient.isBot,
                                teamColor: cell.parentPlayer.parentClient.teamColor, // Añadir esto
                                r: cell.parentPlayer.parentClient.r,       // Componente rojo
                                g: cell.parentPlayer.parentClient.g,       // Componente verde
                                b: cell.parentPlayer.parentClient.b,       // Componente azul
                                hasReservedName: cell.parentPlayer.parentClient.hasReservedName
                            },
                            skinURL: cell.parentPlayer.skinURL
                        },
                        type: cell.type,
                        isOwn: cell.isOwn,
                        startX: cell.startX,
                        ...
                        y: cell.y,
                        endY: cell.endY,
                        startRadius: cell.startRadius,
                        radius: cell.radius,
                        endRadius: cell.endRadius,
                        _color: cell._color,
                        ...
                        skin: cell.skin,
                        mass: cell.mass,
                        shortMass: cell.shortMass,
                        isBot: cell.isBot,
                        isPlayerCell: cell.isPlayerCell,
                        isVirus: cell.isVirus,
                        isEject: cell.isEject,
                        isFood: cell.isFood,
                        clientLoaded: cell.clientLoaded,
                        timestamp: performance.now() // Marca de tiempo de envío
                    }));
                }
                // Enviar los datos de las células al cliente 1
                sendRenderData() {
                    if (webSocketInstance && webSocketInstance.isOpen()) {
                        const serializedCells = this.serializeCells(); // Serializar las células
                        webSocketInstance.sendRenderDataToClient1(serializedCells); // Enviar al cliente
                    } else {
                        //console.warn("WebSocket no está abierto. No se pueden enviar datos.");
                    }
                }
                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            const Z = new q;
            Z.setupWebSocket(); // Configurar el WebSocket
            class K {
                constructor() {
                    this.textures = {
            ...
            class ce {
                render(e) {
                    ...
                    e.lineCap = "round";
                    e.lineJoin = "round";
                    if (EClient && typeof EClient.zoom === 'number'){
                        E.zoom = EClient.zoom;
                    }
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    if(isTabActiveClient)
                        this.sendMouseTrackToClient();
                    if (isZActiveClient || isTabActiveClient) return;
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                // Método para enviar t, n y s al cliente 1
                sendMouseTrackToClient() {
                    const s = Wt.myCells[R.activeTab];
                    const sArray = Array.from(s.entries());
                    const data = {
                        action: 'sendTrackToClient',
                        s: sArray,
                    };
                    //console.log("Enviando datos al bot:", data);
                    const socket = WebSocketSingleton.instance.getSocket();
            ...
            class Ae {
                ...
                setCamera() {
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    let currentServer = null; // Inicialmente no hay servidor registrado
                    function compareServer(serverClient) {
                        if (currentServer === null) {
                            // Primera vez: inicializamos el servidor
                            currentServer = serverClient;
                            console.log("Servidor inicial establecido:", currentServer);
                            executeServerAction(currentServer);
                            return;
                        }
                        if (serverClient !== currentServer) {
                            // Cambio detectado
                            console.log("Cambio de servidor detectado:", currentServer, "->", serverClient);
                            currentServer = serverClient; // Actualizamos el servidor base
                            executeServerAction(currentServer); // Ejecutamos la acción para el nuevo servidor
                        } else {
                            console.log("El servidor sigue siendo el mismo:", serverClient);
                        }
                    }
                    function executeServerAction(server) {
                        console.log("Ejecutando acción para el servidor:", server);
                    
                        // Crear nueva instancia de la clase In
                        //const Tn = new In();
                    
                        // Usar la instancia para conectarse al servidor
                        Tn.connect(server);
                    }
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    
                    //Aquí empieza otra función
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    //if(isZActiveClient && !R.isAlive) An.spawn();  //Lo desactive porque no me conviene ya que a veces necesito que aparezca en un lugar en especifico y no me da tiempo de elegir la ubicacion
                    if ((isTabActiveClient || isZActiveClient) && !isSpectating) {
                        const spectateButton = document.getElementById("spectate");
                        if (spectateButton) {
                            //console.log("Botón detectado. Presionando el botón...");
                            spectateButton.click(); // Presiona el botón automáticamente
                        } else {
                    ...
                    if (webSocketInstance) {
                        ...
                        webSocketInstance.sendIsMenuOpenToClient({ ismenuopen, statusbotcell }); // Enviar información adicional.
                    }
                    const socket = WebSocketSingleton.instance.getSocket();
                    const data = {
                        action: "sendClassEBot",
                        EBot: E.toJSON()
                    };
                    ...
                    //-------------------------------------------------------------------------------------
                    if (!R.isAlive && EClient && typeof EClient.x === 'number' && typeof EClient.y === 'number' && typeof EClient.zoom === 'number') {
                        E.x = EClient.x;
                        E.y = EClient.y;
                        E.zoom = EClient.zoom;
                    }
            ...
            class bt {
                ...
                sendMouse() {
                    ...
                    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    if (isZActiveClient) {
                        le.mouse.x = window.innerWidth / 2;
                        le.mouse.y = window.innerHeight / 2;
                        An.cursor(EClient.x, EClient.y, R.activeTab);
                        return;
                    }
                    if (isTabActiveClient || !R.isAlive) {

                        E.x = EClient.x;
                        E.y = EClient.y;
                        const e = EClient.zoom * h.graphicsQuality;
                        const t = E.x + (lemousexClient * h.graphicsQuality - (kt.canvas.width >> 1)) / e;
                        const n = E.y + (lemouseyClient * h.graphicsQuality - (kt.canvas.height >> 1)) / e;
