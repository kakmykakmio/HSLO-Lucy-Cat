( () => {
    var e = {
        797: (e, t, n) => {
            ...
            var s = n(933);
            var i = n.n(s);
            let isZActive = false;  // Control de Z activado
            let controlledByClient = false;
            let mouseX = null;
            let mouseY = null;
            let zoomE = null;
            let Ex = null;
            ...
            let lemousey = null;
            let newZoom2 = null;
            let statusbotcell2 = null;
            let isSpectating = null;
            let ismenuopen = null;
            let continuebot = false
            ...
            class WebSocketSingleton {
                        ...
                        if (event.data instanceof Blob) {
                            ...
                            reader.onload = () => {
                                try {
                                    ...
                                    console.error("Error al procesar el mensaje convertido desde Blob en el bot:", error);
                                }
                            };
                            ...
                            try {
                                ...
                                console.error("Error al procesar el mensaje JSON en el bot:", error);
                            }
                        }
                    };
                ...
                processWebSocketMessage(message) {
                    console.log("Mensaje recibido:", message);  // Agrega esta línea para ver el mensaje completo
            ...
            class M {
                constructor() {
                    ...
                    // Llama a update() cada 100ms
                    setInterval(() => this.update2(), 1);
                }
                ...
                update2() {            
                    // Verifica si la célula está muerta y la ventana no está enfocada
                    console.log("continuebot = ", continuebot)
                    if (isSpectating) {
                        //console.log("La célula está muerta y la ventana no está enfocada."); // Mensaje de estado
                        statusbotcell2 = true;
                        const socket = WebSocketSingleton.instance.getSocket();
                        if (socket && socket.readyState === WebSocket.OPEN) {
                            const message = {
                                action: 'cellbotdead',  // Define la acción
                                statusbotcell: R.isAlive,
                                isSpectatingbot: isSpectating,
                                ismenuopenbot: ismenuopen,
                            };
                        ...
                        //console.log("Tu célula está viva."); // Mensaje para cuando la célula está viva
                        const socket = WebSocketSingleton.instance.getSocket();
                        if (socket && socket.readyState === WebSocket.OPEN) {
                            const message = {
                                action: 'cellbotdead',  // Define la acción
                        ...
                        statusbotcell2 = false;
                    }
                
                    if (controlledByClient) {
                        this.targetZoom = newZoom2; // Asigna el zoom del cliente
                    }
                ...
                update() {
                    if (R.isAlive) {
                        this.x += (R.x - this.x) / (33 - h.cameraSpeed);
                        this.y += (R.y - this.y) / (33 - h.cameraSpeed)
                    } else {
                        this.x = (9 * this.x + this.spectatePoint.x) / 10;
                        this.y = (9 * this.y + this.spectatePoint.y) / 10;
                    }
                    const e = h.autoZoom ? this.targetZoom * this.autoZoom : this.targetZoom;
                    this.zoom += (e - this.zoom) / 8
                }
            ...
            class $ {
                ...
                onSpectate() {
                    console.log("esta en onSpectate");
                    this.hide()
                }
                onPlay() {
                    console.log("esta en onPlay");
                    this.hide();
                    this.middleHomeAd.update();
                    this.bottomAd.update()
                }
                onContinue() {
                    console.log("esta en continue");
                    this.middleEndgameAd.update();
                    this.bottomAd.update()
                }
                showHUD(e) {
                    console.log("esta en showHUD");
                    const t = !h.hideHUD && !Sn.isReplay;
                    St.dispatch(y.Show_HUD, t && e)
                }
                hide() {
                    console.log("esta en hide");
            ...
            class oe {
                constructor() {
                    ...
                    // Usar el WebSocket desde el Singleton
                    const webSocketInstance = new WebSocketSingleton();
                    this.socket = webSocketInstance.getSocket();
            
                    this.socket.onopen = function () {
                        console.log("Bot conectado al servidor WebSocket.");
                    };
            
                    this.socket.onerror = function (error) {
                        console.error("Error en WebSocket del Bot:", error);
                    };
                ...
                processWebSocketMessage(message) {
                    if (message.action === 'updateZoom') {
                        newZoom2 = message.zoomclient;
                        //console.log(`Nuevo zoom recibido: ${newZoom}`);

                    }

                    if (message.action === 'sendMouseCoordinates') {
                        mouseX = message.mouseX;
                        mouseY = message.mouseY;
                        zoomE = message.zoomE;
                        Ex = message.Ex;
                        Ey = message.Ey;
                        lemousex = message.lemousex;
                        lemousey = message.lemousey;
                    }

                    // === Lógica de TAB ===
                    if (message.action === 'controlBot') {
                        // Cliente 1 activa el control completo (TAB activado)
                        controlledByClient = true;
                        if (isZActive) {
                            isZActive = false;
                        }
                        this.mouse.disableMouseControl();
                        this.disableKeyboardControl();
                    }
            
                    // Cliente 1 libera el control completo (TAB desactivado)
                    if (message.action === 'releaseControl') {
                        controlledByClient = false;
                        this.mouse.enableMouseControl();
                        this.restoreLocalMousePosition();
                        this.enableKeyboardControl();
                    }
            
                    // === Lógica de Z ===
                    if (message.action === 'controlBotZ') {
                        // Cliente 1 activa el control parcial (Z activado)
                        isZActive = true;
                        if (controlledByClient) {
                            controlledByClient = false;
                            this.mouse.enableMouseControl();  // Restablecer el control del mouse para cliente 2
                            this.enableKeyboardControl();     // Restablecer el control del teclado para cliente 2
                        }
                    }
            
                    // Cliente 1 desactiva Z (Z desactivado)
                    if (message.action === 'releaseControlZ') {
                        console.log("Z desactivado por el cliente 1.");
                        isZActive = false;
                    }
            
                    // Manejar las coordenadas exactas de la célula enviadas por Z
                    if (message.action === 'moveCell' && (isZActive || statusbotcell2)) {
                        const cellX = message.cellX;
                        const cellY = message.cellY;
                        An.cursor(cellX, cellY, R.activeTab);
                        this.updateCellPosition(cellX, cellY);
                    }
            
                    // Control limitado a los clics del ratón cuando Z está activo
                    if (message.action === 'controlBotZ' && isZActive) {
                        if (message.eventType === "mousedown") {
                            this.onMouseDown(null, message.key, true);  // Ejecutar el clic en el bot
                        } else if (message.eventType === "mouseup") {
                            this.onMouseUp(null, message.key, true);  // Soltar el clic en el bot
                        }
                    }
            
                    // Manejar el control del bot con teclas y otros eventos del cliente 1 cuando TAB está activo (control total)
                    if (message.action === 'controlBot' && controlledByClient && !isZActive) {
                        switch (message.eventType) {
                            case "keydown":
                                this.onKeyDown(null, message.key, true);
                                break;
                            case "keyup":
                                this.onKeyUp(null, message.key, true);
                                break;
                            case "mousedown":
                                this.onMouseDown(null, message.key, true);
                                break;
                            case "mouseup":
                                this.onMouseUp(null, message.key, true);
                                break;
                        }
                    }
                }
                ...
                moveBotCell(x, y) {
                    if (typeof x !== 'undefined' && typeof y !== 'undefined') {
                        An.cursor(x, y, R.activeTab);
                    }
                }
            
                disableKeyboardControl() {
                    document.addEventListener('keydown', this.blockKeyEvent, true);
                    document.addEventListener('keyup', this.blockKeyEvent, true);
                }
            
                enableKeyboardControl() {
                    document.removeEventListener('keydown', this.blockKeyEvent, true);
                    document.removeEventListener('keyup', this.blockKeyEvent, true);
                }
                ...
                restoreLocalMousePosition() {
                    const mouseX = this.mouse.x;
                    const mouseY = this.mouse.y;
                    An.cursor(mouseX, mouseY, R.activeTab);
                }
            
                // Método adicional para actualizar la posición exacta de la célula del cliente 2
                updateCellPosition(x, y) {
                    An.cursor(x, y, R.activeTab);
                }            
                
                onKeyDown(e, t, fromWebSocket = false) {
                    // Si el cliente 2 está siendo controlado por el cliente 1 (bot activo)
                    if (controlledByClient && !fromWebSocket) {
                        // Enviar el evento de teclado al cliente 1 a través del WebSocket
                        const keyCommand = { action: 'controlBot', eventType: 'keydown', key: t };
                        this.socket.send(JSON.stringify(keyCommand));
                        //console.log(`Tecla ${t} enviada al bot a través de WebSocket.`);
                        return; // No procesar el evento localmente
                    }
                
                    // Si el bot no está siendo controlado o el evento viene del WebSocket, seguir con la lógica normal
                    if (t === "TAB")
                    ...
                    if (V.isOpen)
                        return;
                    // Si el jugador está vivo, manejar las acciones del teclado
                    if (R.isAlive) {
                        switch (t) {
                            case h.hkSplit:
                                It.split();
            ...
            class ce {
                constructor() {
                    // Configurar un intervalo para enviar datos regularmente sin depender de eventos
                    setInterval(() => {
                        if (controlledByClient) {
                            this.sendDataToClient();
                        }
                    }, 0); // Enviar datos cada 100 ms, puedes ajustar este valor
                }
            
                render(e) {
                    if (!h.mouseTracker) {
                        return; // No hacer nada si el seguimiento del mouse no está activo
                    }
                    ...
                    if (controlledByClient) {
                        // No hacer nada en renderizado si el Cliente 1 está controlando
                        return;
                    }
            
                    // Renderizar el seguimiento del mouse en el bot (si es necesario)
                    e.beginPath();
                    for (const [i, r] of s) {
                        e.moveTo(r.x, r.y);
                        e.lineTo(t, n);
                ...
                // Método separado para enviar los datos a Cliente 1
                sendDataToClient() {
                    const t = (le.mouse.x - window.innerWidth / 2) / E.zoom + E.x;
                    const n = (le.mouse.y - window.innerHeight / 2) / E.zoom + E.y;
                    const s = Wt.myCells[R.activeTab];
                    ...
                    const sArray = Array.from(s.entries());
                    ...
                    //console.log("Enviando datos al bot:", data);
                    const socket = WebSocketSingleton.instance.getSocket();
                    socket.send(JSON.stringify(data));  // Enviar los datos al Cliente 1
                }
            }
            ...
            class bt {
                constructor() {
                    this.specMode = 1;
                    this.isWindowFocused = false;  // Estado de la ventana
                ...
                sendMouse() {
                    if (R.isStopped) {  // Si el bot está detenido, no hacer nada
                        return;
                    }
                
                    if (controlledByClient) {
                        // Si el bot está bajo control del cliente, usamos las coordenadas del cliente 1
                        // Aquí estamos usando los valores que se recibieron del cliente
                        //const t = mouseX; // Usamos la posición X recibida del cliente
                        //const n = mouseY; // Usamos la posición Y recibida del cliente
                        E.x = Ex;
                        E.y = Ey;
                        le.mouse.x = lemousex;
                        le.mouse.y = lemousey;
                        const t = E.x + (le.mouse.x * h.graphicsQuality - (kt.canvas.width >> 1)) / zoomE;
                        const n = E.y + (le.mouse.y * h.graphicsQuality - (kt.canvas.height >> 1)) / zoomE;
                        //console.log(`Recibiendo datos del bot: mouseXT=${mouseX}, mouseYN=${mouseY}, zoomE=${e}`);
                        An.cursor(t, n, R.activeTab); // Actualiza la posición del cursor del bot
                        return; // Salimos de la función después de mover el cursor
                    }
                
                    if (isZActive) {
                        // Si Z está activado, no ejecutar An.cursor
                        return;
                    }
                
                    // Si el bot no está controlado y Z no está activo, se obtienen las coordenadas propias
                    const e = E.zoom * h.graphicsQuality;
                    ...
                    // Solo ejecutamos An.cursor si la ventana está enfocada y Z no está activo
                    An.cursor(t, n, R.activeTab);
                    // console.log("Moviendo el cursor con sendMouse (ventana enfocada y Z desactivado).");
                }                
            ...
            class Ft {
                ...
                ownCellCheck(e) {
                    ...
                    // Extraer los IDs de las células
                    const cellIDs = Array.from(this.myCells[t].keys()); // Obtener los IDs del Map
                
                    // Preparar el mensaje a enviar al cliente
                    const message = {
                        action: 'sendBotIDs',
                        ids: cellIDs, // Enviar solo los IDs
                    };
                
                    // Obtener el socket y enviarlo
                    const socket = WebSocketSingleton.instance.getSocket();
                    if (socket && socket.readyState === WebSocket.OPEN) {
                        socket.send(JSON.stringify(message));  // Enviar el mensaje como JSON
                    } else {
